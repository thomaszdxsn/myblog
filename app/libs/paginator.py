...
class Paginator(object):
    ...
    def page(self, page_number):
        ...
        return Page(
            self.query_set.offset(offset).limit(self.per_page_limit).all(),
            page_number, self)
    ...
    def __get_count(self):
        ...
        if self.__count is None:
            ...
            count_query = self.optional_count_query_set.statement.with_only_columns(
                [func.count()])
            self.__count = self.optional_count_query_set.session.execute(
                count_query).scalar()
        return self.__count
        ...
        if self.__total_pages is None:
            if self.count == 0 and not self.allow_empty_first_page:
                ...
                self.__total_pages = int(
                    ceil(hits / float(self.per_page_limit)))
